...
class _RegistrationScreenState extends State<RegistrationScreen> {
  ...
  Future<void> _submitRegistration() async {
    ...
    if (email.isEmpty || !email.contains('@')) {
      _showError(_t('Please enter a valid email address.',
          'يرجى إدخال بريد إلكتروني صحيح.'));
      return;
    }
    if (phone.isEmpty) {
      _showError(_t('Please enter a valid mobile number.',
          'يرجى إدخال رقم هاتف متحرك صحيح.'));
      return;
    }
    if (password.length < 6) {
      _showError(_t('Password must be at least 6 characters.',
    ...
    if (!agreed) {
      _showError(_t('Please agree to the Terms & Conditions.',
    ...
    try {
      ...
      // Upload documents only after authentication. Storage rules scope every
      // registration file to the signed-in technician's uid.
      try {
        for (final entry in _pendingBytes.entries) {
          final fileName = _pendingNames[entry.key] ?? '${entry.key}.jpg';
          final url = await StorageService.instance.uploadTechnicianFile(
            category: 'registration/${entry.key}',
            fileName: fileName,
            bytes: entry.value,
          );
          if (entry.key == 'emp_emirates_id') _empEmiratesIdUrl = url;
          if (entry.key == 'emp_profile_photo') _empProfilePhotoUrl = url;
          if (entry.key == 'comp_trade_license') _compTradeLicenseUrl = url;
          if (entry.key == 'comp_logo') _compLogoUrl = url;
        }
      } catch (_) {
      ...
      final Map<String, dynamic> registrationData = isEmp
          ? {
              ...
              'emirate': _empEmirate,
              'area': _empArea.text.trim().isEmpty
                  ? _empEmirate
                  : _empArea.text.trim(),
              'documents': {
          ...
          : {
              ...
              'areas_covered': _compAreasCovered.text.trim().isEmpty
                  ? _compEmirate
                  : _compAreasCovered.text.trim(),
              'documents': {
      ...
      if (mounted) {
        ...
        if (e.code == 'email-already-in-use') {
          _showError(_t('This email is already registered. Please sign in.',
              'هذا البريد مسجل بالفعل. يرجى تسجيل الدخول.'));
        } else if (e.code == 'weak-password') {
          _showError(_t(
              'The password provided is too weak.', 'كلمة المرور ضعيفة جداً.'));
        } else {
          _showError(e.message ??
              _t('Registration error occurred.', 'حدث خطأ أثناء التسجيل.'));
        }
      }
    } catch (e) {
      if (mounted) {
        ...
        Padding(
          ...
          child: Row(
            children: [
                ...
                child: Center(
                  child: RichText(
                    text: const TextSpan(
                      ...
                      children: [
                        TextSpan(text: 'KBI'),
                        TextSpan(text: '.', style: TextStyle(color: kbiBrand)),
                      ],
                    ),
                  ),
                ),
              ...
              TextButton(
                onPressed: () =>
                    widget.onLocaleChanged(Locale(_isArabic ? 'en' : 'ar')),
              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      _t('Create your profile to receive high-value service orders.',
                          'أنشئ حسابك المهني وابدأ في استقبال طلبات الصيانة.'),
                      textAlign: TextAlign.center,
                      style: const TextStyle(
                          fontSize: 13.5,
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _buildSectionHeading(
                              _t('1. Account Information', '1. معلومات الحساب'),
                          ...
                          if (_accountType == 'employee') ...[
                            ...
                            _buildInput(
                              ...
                              label:
                                  _t('Mobile Number *', 'رقم الهاتف المتحرك *'),
                            ...
                            _buildInput(
                              ...
                              label: _t('Manager / Owner Name *',
                                  'اسم المدير / المالك *'),
                              icon: Icons.person_pin_outlined,
                            ),
                            _buildInput(
                              controller: _compTradeLicenseNum,
                              label: _t('Trade License Number *',
                                  'رقم الرخصة التجارية *'),
                          ...
                          _buildSectionHeading(
                              _t('2. Service & Coverage',
                                  '2. الخدمات ونطاق التغطية'),
                              Icons.build_circle_outlined),
                          const SizedBox(height: 14),

                          // Services / Specialization Chips
                          Text(
                            _t('Specializations / Services Offered *',
                          ...
                          Row(
                            children: [
                              ...
                              Expanded(
                                child: _buildDropdown(
                                  label: _accountType == 'employee'
                                      ? _t('Experience *', 'الخبرة *')
                                      : _t('Techs Count *', 'عدد الفنيين *'),
                                  value: _accountType == 'employee'
                                      ? _empExperience
                                      : _compNumTechs,
                          ...
                          _buildInput(
                            controller: _accountType == 'employee'
                                ? _empArea
                                : _compAreasCovered,
                            label: _t('Specific Areas Covered *',
                                'المناطق التي تغطيها *'),
                            icon: Icons.location_on_outlined,
                            hint: _t('e.g. Downtown, Marina, Business Bay',
                                'مثال: وسط المدينة، مارينا، الخليج التجاري'),
                          ),
                          ...
                          if (_accountType == 'employee') ...[
                            _buildUploadCard(
                              label: _t('Emirates ID (Front / Back)',
                            ...
                            _buildUploadCard(
                              label: _t(
                                  'Personal Profile Photo', 'الصورة الشخصية'),
                            ...
                            _buildUploadCard(
                              label: _t(
                                  'Trade License Copy', 'نسخة الرخصة التجارية'),
                ...
                child: Row(
                  ...
                  children: [
                    Icon(
                      ...
                      size: 20,
                      color: isEmp ? kbiBrand : const Color(0xFF64748B),
                    ),
                      ...
                      child: FittedBox(
                        ...
                        child: Text(
                          ...
                          style: TextStyle(
                            fontSize: 13,
                            fontWeight:
                                isEmp ? FontWeight.w800 : FontWeight.w600,
                            color: isEmp
  ...
  Widget _buildSectionHeading(String title, IconData icon) {
    return Row(
      children: [
        Icon(icon, size: 20, color: kbiBrand),
        const SizedBox(width: 8),
        Expanded(
          child: Text(
            title,
          ...
          TextField(
            ...
            decoration: InputDecoration(
              ...
              focusedBorder: OutlineInputBorder(
                borderRadius: BorderRadius.circular(14),
                borderSide: const BorderSide(color: kbiBrand, width: 1.6),
              ),
            ),
          ),
        ],
  ...
  Widget _buildSpecializationChips() {
    final list =
        _accountType == 'employee' ? _empSpecializations : _compServices;
    const options = [
      'Mobile Repair',
      'Laptop Repair',
      'Printer Repair',
    ...
    return Wrap(
      ...
      children: options.map((opt) {
        ...
        return FilterChip(
          ...
          selectedColor: kbiBrand.withValues(alpha: 0.12),
          checkmarkColor: kbiBrand,
          backgroundColor: const Color(0xFFF8FAFC),
          side:
              BorderSide(color: selected ? kbiBrand : const Color(0xFFE2E8F0)),
          labelStyle: TextStyle(
            fontSize: 12.5,
            fontWeight: selected ? FontWeight.w700 : FontWeight.w500,
            color: selected ? kbiBrand : const Color(0xFF475569),
          ),
          shape:
        ...
        child: Row(
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                color:
                    hasFile ? const Color(0xFFDCFCE7) : const Color(0xFFE2E8F0),
              ...
              child: Icon(
                ...
                size: 20,
                color:
                    hasFile ? const Color(0xFF16A34A) : const Color(0xFF64748B),
              ),
            ),
            const SizedBox(width: 12),
            Expanded(
                ...
                children: [
                  ...
                  Text(
                    isUploading
                        ...
                        : (hasFile
                            ? (fileName ??
                                _t('Document attached', 'المستند مرفق'))
                            : _t('Tap to upload PDF, PNG or JPG',
                                'اضغط لرفع ملف PDF أو صورة')),
                    style: TextStyle(
                      fontSize: 11.5,
                      color: hasFile
            ...
            if (isUploading)
              const SizedBox(
                ...
                height: 18,
                child:
                    CircularProgressIndicator(strokeWidth: 2, color: kbiBrand),
              )
            else
              Text(
                hasFile ? _t('Change', 'تغيير') : _t('Browse', 'استعراض'),
                style: const TextStyle(
                    fontSize: 12, fontWeight: FontWeight.w700, color: kbiBrand),
              ),
          ],
        ),
      ),
