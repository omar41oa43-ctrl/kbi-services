...
class ApprovalPendingScreen extends StatefulWidget {
  ...
  const ApprovalPendingScreen({super.key, required this.onLocaleChanged, required this.locale});
...
class _ApprovalPendingScreenState extends State<ApprovalPendingScreen> {
  ...
  final Map<String, List<String>> _subSkillsMapping = {
    ...
    'CCTV Installation': ['IP Camera', 'Analog Camera', 'DVR/NVR', 'PTZ Camera'],
    'PlayStation Repair': ['PS4', 'PS5', 'Controllers'],
    'Xbox Repair': ['Xbox One', 'Xbox Series X/S', 'Controllers'],
    'Networking': ['Router Configuration', 'Switch Installation', 'Cabling', 'Access Points'],
    'Smart Home': ['Ring Doorbell', 'Smart Lights', 'Smart Thermostat', 'Smart Locks'],
  ...
  Map<String, dynamic> _buildPayload(String status) {
    return {
      ...
      'experience_main_skill': _selectedSubSkills.isNotEmpty ? _selectedSubSkills.join(', ') : '',
      ...
      'latitude': double.tryParse(_latitude.text) ?? 24.4539,
      'longitude': double.tryParse(_longitude.text) ?? 54.3773,
  ...
  Future<void> _submit() async {
    if (!_agreeTerms || !_agreeCommission || !_agreeBehavior || !_understandApproval) {
      setState(() {
      ...
      builder: (context, child) {
        return Theme(
          data: Theme.of(context).copyWith(
            ...
            dialogBackgroundColor: const Color(0xFF0B0F14),
          ),
          child: child!,
        );
      },
          ...
          builder: (context, setDialogState) {
            return AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  const Text('An OTP code has been sent to your number. Enter "123456" to verify.', style: TextStyle(color: Colors.white70)),
                ...
                ElevatedButton(
                  onPressed: loading ? null : () async {
                    ...
                    if (otp.text == '123456') {
                      ...
                      setDialogState(() {
                        loading = false;
                        err = 'Invalid OTP code. Enter 123456';
                      });
                    }
                  },
                  child: loading
    ...
    try {
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        await Future.doWhile(() async {
          ...
          if (mounted) {
            setState(() {
              _photoUploadProgress = ticks / 10.0;
            });
          }
          return ticks < 10;
        });
        ...
        if (file.bytes != null) {
          ...
          dataUrl = 'data:image/${file.extension ?? "png"};base64,$base64String';
        } else {
          dataUrl = 'https://api.dicebear.com/7.x/bottts/png?seed=uploaded';
        }
    ...
    try {
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        await Future.doWhile(() async {
          ...
          if (mounted) {
            setState(() {
              _uploadProgress[docKey] = ticks / 10.0;
            });
          }
          return ticks < 10;
        });
        ...
        if (file.bytes != null) {
          ...
          dataUrl = 'https://firebasestorage.googleapis.com/v0/b/kbi2-f4f19.appspot.com/o/mock_${docKey}.pdf?alt=media';
        }

        if (mounted) {
          setState(() {
            _uploading[docKey] = false;
            if (docKey == 'emirates_id') _emiratesIdUrl = dataUrl;
            if (docKey == 'passport') _passportUrl = dataUrl;
            if (docKey == 'visa') _visaUrl = dataUrl;
            if (docKey == 'cv') _cvUrl = dataUrl;
            if (docKey == 'driving_license') _drivingLicenseUrl = dataUrl;
            if (docKey == 'certificate') _certificateUrl = dataUrl;
          });
          _saveDraft();
        }
      }
  ...
  Widget _buildStepHeader() {
    final steps = ['Personal', 'Skills', 'Areas', 'Verify', 'Payout', 'Confirm'];
    return Padding(
                  ...
                  children: [
                    CircleAvatar(
                      ...
                      child: Text(
                        '${index + 1}',
                        style: TextStyle(
                          color: active || completed ? Colors.black : Colors.white60,
  ...
  Widget _buildStep1() {
    return Column(
      ...
      children: [
              ...
              children: [
                CircleAvatar(
                  ...
                  child: _profilePhotoUrl == null
                      ? _photoUploading
                          ? CircularProgressIndicator(value: _photoUploadProgress)
                          : const Icon(Icons.add_a_photo, color: Colors.cyan, size: 28)
                      : null,
                ),
                const SizedBox(height: 8),
                const Text('Choose Profile Photo *', style: TextStyle(fontSize: 12, color: Colors.white54)),
              ],
            ),
          ),
        ),
        const SizedBox(height: 24),
        const Text('Contact Information', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.cyanAccent)),
        ...
        Container(
          ...
          child: Column(
            children: [
              ...
              const SizedBox(height: 12),
              TextField(controller: _whatsapp, keyboardType: TextInputType.phone, decoration: const InputDecoration(labelText: 'WhatsApp Number *')),
              const SizedBox(height: 12),
        ...
        const SizedBox(height: 24),
        const Text('Security Credentials', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.cyanAccent)),
        const SizedBox(height: 10),
        Container(
          padding: const EdgeInsets.all(16),
          decoration: BoxDecoration(
        ...
        const SizedBox(height: 24),
        const Text('Identity & Preferences', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.cyanAccent)),
        const SizedBox(height: 10),
        Container(
          padding: const EdgeInsets.all(16),
          decoration: BoxDecoration(
          ...
          child: Column(
            children: [
              ...
              const SizedBox(height: 12),
              const Text('Languages Spoken *', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.white54)),
              const SizedBox(height: 8),
              Row(
                children: [
                  Expanded(
              ...
              const Text('Suggestions:', style: TextStyle(fontSize: 11, color: Colors.white38)),
              const SizedBox(height: 4),
              Wrap(
                spacing: 6.0,
                runSpacing: 6.0,
                children: ['English', 'Arabic', 'Urdu', 'Hindi', 'Tagalog'].map((lang) {
                  ...
                  return GestureDetector(
                    ...
                    child: Chip(
                      label: Text(lang, style: TextStyle(fontSize: 11, color: contains ? Colors.black : Colors.white70)),
                      backgroundColor: contains ? Colors.cyan : const Color(0xFF0F141C),
  ...
  Widget _buildStep2() {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Select Skills (Choose one or more) *', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.cyanAccent)),
        const SizedBox(height: 10),
        Container(
          padding: const EdgeInsets.all(16),
          decoration: BoxDecoration(
        ...
        const Text('Professional Experience', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.cyanAccent)),
        const SizedBox(height: 10),
        Container(
          padding: const EdgeInsets.all(16),
          decoration: BoxDecoration(
          ...
          child: Column(
            children: [
              DropdownButtonFormField<String>(
                ...
                dropdownColor: const Color(0xFF0B0F14),
                items: const [
                  DropdownMenuItem(value: 'Less than 1 year', child: Text('Less than 1 year')),
        ...
        const Text('Preferences & Equipment', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.cyanAccent)),
        const SizedBox(height: 10),
        Container(
          decoration: BoxDecoration(
            color: const Color(0xFF0D1117),
          ...
          child: Column(
            children: [
              SwitchListTile(
                title: const Text('Owns a Vehicle', style: TextStyle(fontSize: 14)),
                value: _ownVehicle,
                onChanged: (v) => setState(() => _ownVehicle = v),
              ),
              const Divider(height: 1, color: Colors.white10),
              SwitchListTile(
                title: const Text('Owns Repair Tools', style: TextStyle(fontSize: 14)),
                value: _ownTools,
                onChanged: (v) => setState(() => _ownTools = v),
              ),
              const Divider(height: 1, color: Colors.white10),
              SwitchListTile(
                title: const Text('Can Work On-site', style: TextStyle(fontSize: 14)),
  ...
  Widget _buildStep3() {
    final areas = ['Abu Dhabi', 'Al Reem Island', 'Khalifa City', 'MBZ City', 'Mussafah', 'Yas Island', 'Al Raha', 'Saadiyat', 'Baniyas', 'Al Ain'];
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Service Areas Covered', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.cyanAccent)),
        const SizedBox(height: 10),
        Container(
          padding: const EdgeInsets.all(16),
          decoration: BoxDecoration(
        ...
        const SizedBox(height: 24),
        const Text('GPS Location Coordinates', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.cyanAccent)),
        const SizedBox(height: 10),
        Container(
          padding: const EdgeInsets.all(16),
          decoration: BoxDecoration(
          ...
          child: Column(
            children: [
              Row(
                children: [
                  Expanded(child: TextField(controller: _latitude, readOnly: true, decoration: const InputDecoration(labelText: 'Latitude'))),
                  const SizedBox(width: 12),
                  Expanded(child: TextField(controller: _longitude, readOnly: true, decoration: const InputDecoration(labelText: 'Longitude'))),
                ],
              ),
              const SizedBox(height: 16),
              SizedBox(
  ...
  Widget _buildStep4() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Upload Verification Documents', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.cyanAccent)),
        const SizedBox(height: 10),
        Container(
          padding: const EdgeInsets.all(16),
          decoration: BoxDecoration(
          ...
          child: Column(
            children: [
              _buildDocRow('Emirates ID Copy *', _emiratesIdUrl, 'emirates_id', true),
              _buildDocRow('CV / Resume Copy *', _cvUrl, 'cv', true),
              _buildDocRow('Driving License (Optional)', _drivingLicenseUrl, 'driving_license', false),
            ],
          ),
        ),
      ],
  ...
  Widget _buildStep5() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Payout Option Settings', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.cyanAccent)),
        const SizedBox(height: 10),
        Container(
          padding: const EdgeInsets.all(16),
          decoration: BoxDecoration(
          ...
          child: Column(
            children: [
              ...
              if (_paymentMethod == 'Bank Transfer') ...[
                ...
                const SizedBox(height: 12),
                TextField(controller: _iban, decoration: const InputDecoration(labelText: 'IBAN Number *')),
                const SizedBox(height: 12),
  ...
  Widget _buildStep6() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Legal Agreements & Policies', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.cyanAccent)),
        const SizedBox(height: 10),
        Container(
          decoration: BoxDecoration(
            color: const Color(0xFF0D1117),
          ...
          child: Column(
            children: [
              CheckboxListTile(
                title: const Text('I agree to the Terms & Conditions of KBI.Services', style: TextStyle(fontSize: 13)),
                value: _agreeTerms,
                onChanged: (v) => setState(() => _agreeTerms = v ?? false),
              ),
              const Divider(height: 1, color: Colors.white10),
              CheckboxListTile(
                title: const Text('I agree to KBI.Services commission policy', style: TextStyle(fontSize: 13)),
                value: _agreeCommission,
                onChanged: (v) => setState(() => _agreeCommission = v ?? false),
              ),
              const Divider(height: 1, color: Colors.white10),
              CheckboxListTile(
                title: const Text('I agree to maintain professional behavior at all times', style: TextStyle(fontSize: 13)),
                value: _agreeBehavior,
                onChanged: (v) => setState(() => _agreeBehavior = v ?? false),
              ),
              const Divider(height: 1, color: Colors.white10),
              CheckboxListTile(
                title: const Text('I understand my account requires admin approval before receiving orders', style: TextStyle(fontSize: 13)),
                value: _understandApproval,
                onChanged: (v) => setState(() => _understandApproval = v ?? false),
              ),
            ],
          ),
        ),
  ...
  bool _validateCurrentStep() {
    ...
    if (_currentStep == 0) {
      if (_name.text.isEmpty || _phone.text.isEmpty || _email.text.isEmpty || _nationality.text.isEmpty || _dob.text.isEmpty) {
    ...
    if (_currentStep == 4) {
      if (_paymentMethod == 'Bank Transfer') {
        if (_bankName.text.isEmpty || _iban.text.isEmpty || _accountHolder.text.isEmpty) {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      stream: FirebaseFirestore.instance.collection('technician_requests').doc(uid).snapshots(),
      builder: (context, snap) {
        ...
        // Show application success / status page if pending
                    ...
                    child: Card(
                      child: Padding(
                        ...
                        child: Column(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            const Icon(Icons.verified_user_outlined, color: Colors.cyan, size: 68),
                            ...
                            const SizedBox(height: 14),
                            const Text(
                              'Thank you for registering with KBI.Services.\n\nYour application is currently under review.\n\nOur verification team will review your information within 24 hours.\n\nYou will receive a WhatsApp message once your account has been approved.',
                              textAlign: TextAlign.center,
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                Container(
                                  ...
                                  child: const Text('Pending Approval', style: TextStyle(color: Colors.yellow, fontSize: 13, fontWeight: FontWeight.bold)),
                                )
                              ],
                            )
                          ],
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                ElevatedButton(
                                  ...
                                  child: _loading
                                      ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2))
                                      : Text(_currentStep < 5 ? 'Next' : 'Submit'),
                                ),
                              ],
                            ),
                          ],
