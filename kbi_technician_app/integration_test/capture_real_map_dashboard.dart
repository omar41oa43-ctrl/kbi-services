...
void main() {
  ...
  testWidgets('Create tech and capture real map distance dashboard',
      (tester) async {
    final timestamp = DateTime.now().millisecondsSinceEpoch;
    final techEmail = 'tech_map_$timestamp@kbi.test';
    ...
    // Create active job directly in `orders` collection (allowed by signedIn tech rule)
    batch.set(
    ...
    // Save high-definition screenshot of dashboard with real map
    final dashBytes = await binding.takeScreenshot('dashboard_real_map_hd');
    await File('$_artifactDir/dashboard_real_map_hd.png').writeAsBytes(dashBytes);
    debugPrint('=== DASHBOARD REAL MAP HD SAVED SUCCESSFULLY ===');
  });
}
