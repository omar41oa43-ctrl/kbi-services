...
import { AlertCircle, ArrowRight, CalendarDays, CheckCircle2, Clock3, LogOut, MapPin, RefreshCw, Settings2, Truck, Wrench } from "lucide-react"
...
export default function TechnicianAppPage() {
  ...
  return <main className="min-h-screen bg-[#f6f9fb] pb-8 text-slate-950"><div className="mx-auto w-full max-w-xl px-4 pb-10 pt-8"><header className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-cyan-700">KBI Field Ops</p><h1 className="mt-1 text-3xl font-black tracking-[-0.05em]">Good to see you.</h1><p className="mt-1 text-sm text-slate-500">Your day at a glance</p></div><div className="flex items-center gap-2"><button onClick={() => user && void loadJobs(user)} className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white text-slate-600" aria-label="Refresh jobs"><RefreshCw className={`h-4 w-4 ${refreshing ? "animate-spin" : ""}`} /></button><button onClick={() => void signOut(auth)} className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white text-slate-600" aria-label="Sign out"><LogOut className="h-4 w-4" /></button></div></header>
      ...
      <section className="mt-6"><div className="mb-3 flex items-center justify-between"><h2 className="text-lg font-black tracking-tight">Current job</h2><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-bold text-emerald-700">{activeJob ? "Live" : "Standby"}</span></div>{activeJob ? <JobCard job={activeJob} featured /> : <div className="rounded-[1.5rem] border border-dashed border-slate-300 bg-white p-6 text-center"><div className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-cyan-50 text-cyan-700"><Wrench className="h-6 w-6" /></div><p className="mt-3 font-extrabold">No active job</p><p className="mt-1 text-sm text-slate-500">New assignments will appear here automatically.</p></div>}</section>
      <section className="mt-7"><div className="mb-3 flex items-center justify-between"><h2 className="text-lg font-black tracking-tight">Requests to review</h2><span className="text-xs font-bold text-slate-400">{availableJobs.length} total</span></div><div className="space-y-3">{availableJobs.length ? availableJobs.map((job) => <JobCard key={job.id} job={job} />) : <p className="rounded-2xl border border-slate-200 bg-white p-5 text-sm text-slate-500">You’re all caught up.</p>}</div></section>
      <nav className="mt-8 grid grid-cols-3 gap-2"><div className="flex items-center gap-2 rounded-xl bg-cyan-50 p-3 text-xs font-bold text-cyan-800"><CalendarDays className="h-4 w-4" />Schedule</div><div className="flex items-center gap-2 rounded-xl bg-violet-50 p-3 text-xs font-bold text-violet-800"><Truck className="h-4 w-4" />Navigation</div><div className="flex items-center gap-2 rounded-xl bg-slate-100 p-3 text-xs font-bold text-slate-700"><Settings2 className="h-4 w-4" />Settings</div></nav>
    </div></main>
}
...
function JobCard({ job, featured = false }: { job: Job; featured?: boolean }) {
  ...
  return <article className={`rounded-[1.5rem] border p-5 shadow-sm ${featured ? "border-cyan-200 bg-cyan-50" : "border-slate-200 bg-white"}`}><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-400">{job.orderNumber || `Job #${job.id.slice(0, 8)}`}</p><h3 className="mt-1 text-lg font-black">{job.device || "Device repair"}</h3><p className="mt-1 text-sm font-semibold text-slate-600">{job.service || "On-site service"} · {customer}</p></div><span className="rounded-full bg-white px-2.5 py-1 text-[11px] font-bold text-cyan-700 shadow-sm">{statusLabel[status] || status}</span></div><div className="mt-4 grid gap-2 text-xs font-medium text-slate-600"><span className="flex items-center gap-2"><MapPin className="h-4 w-4 text-cyan-600" />{location}</span><span className="flex items-center gap-2"><Clock3 className="h-4 w-4 text-cyan-600" />{job.scheduledAt ? new Date(job.scheduledAt).toLocaleString() : "Schedule to be confirmed"}</span></div><div className="mt-5 flex gap-2"><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(location)}`} target="_blank" rel="noreferrer" className="flex h-10 flex-1 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-800"><MapPin className="h-4 w-4" />Navigate</a><button className="flex h-10 flex-1 items-center justify-center gap-2 rounded-xl bg-slate-950 text-xs font-bold text-white">Open job <ArrowRight className="h-4 w-4" /></button></div></article>
}
